...
def train(loader: DataLoader, model: Module, optimizer: Optimizer, scheduler, criterion, num_classes: int, n_epochs: int,
    ...
    for epoch in range(first_epoch + 1, n_epochs + 1):
        ...
        for i, (x, y) in enumerate(tqdm(loader, leave=False)):
            epoch_loss += optimization_step(x, y, model, optimizer, scheduler, criterion, num_classes, clip_norm)[0]
...
def optimization_step(x, y, model, optimizer, scheduler, criterion, num_classes, clip_norm, reverse=None):
    optimizer.zero_grad()
